...
def run(seed):
    ...
    ue=nn.Embedding(num_u,n_dim); it=Item(0.3)
    opt=Adam(list(ue.parameters())+list(it.parameters()),lr=1e-3,weight_decay=1e-4)
    ...
    ue.load_state_dict(best['u']); it.load_state_dict(best['i']); ue.eval(); it.eval()
    with torch.no_grad():
        ...
        for u in users:
            ...
            _,idx=torch.topk(sc,50); idx=idx.tolist()
            tset=te_by_user[u]
            rec += len(set(idx)&tset)/len(tset)
            dcg=sum(1/math.log2(i+2) for i,m in enumerate(idx) if m in tset)
            idcg=sum(1/math.log2(k+2) for k in range(min(len(tset),50)))
            ndcg += dcg/idcg
    return rec/len(users)*100, ndcg/len(users)*100

print('seed | Recall@50  NDCG@50', flush=True)
R=[]; N=[]
for seed in [1,2,3]:
    r,n=run(seed); R.append(r); N.append(n)
    print(f'{seed:4d} | {r:8.2f}  {n:7.2f}', flush=True)
import statistics as st
print('---', flush=True)
print(f'Recall@50: mean {st.mean(R):.2f} std {st.pstdev(R):.2f}', flush=True)
print(f'NDCG@50:   mean {st.mean(N):.2f} std {st.pstdev(N):.2f}', flush=True)
