...
def run(seed, posfix):
    ...
    for i in range(200000):
        ...
        if posfix:
            d=torch.arange(len(ix)); s[d,d]=s[d,d]+torch.log(Q[tm[ix]])
...
print('seed | standard  posfix  delta', flush=True)
d=[]; a=[]; b=[]
for seed in [1,2,3]:
    rs=run(seed,False); rp=run(seed,True)
    d.append(rp-rs); a.append(rs); b.append(rp)
    print(f'{seed:4d} | {rs:6.2f}  {rp:6.2f}  {rp-rs:+.2f}', flush=True)
import statistics as st
print('---', flush=True)
print(f'standard: mean {st.mean(a):.2f} std {st.pstdev(a):.2f}', flush=True)
print(f'posfix:   mean {st.mean(b):.2f} std {st.pstdev(b):.2f}', flush=True)
print(f'delta(posfix-standard): mean {st.mean(d):+.2f} std {st.pstdev(d):.2f}', flush=True)
